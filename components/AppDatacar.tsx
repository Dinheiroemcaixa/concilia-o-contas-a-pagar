  ...
  return (
    <div className={'dc-app' + (props.isDark ? ' dc-dark' : '')}>
      <div className="dc-thm">
        <div className="dc-icon-btn" onClick={props.toggleTema} title="Alternar tema">
          {props.isDark ? iconSun : iconMoon}
        </div>
      </div>
      <div className="dc-lw">
        <div className="dc-lc">
          ...
          {tela === 'login' && (
            <div>
              ...
              <input className="dc-linp" type="text" placeholder="seu.usuario" value={loginUser} onChange={function (e) { setLoginUser(e.target.value) }} onKeyDown={function (e) { if (e.key === 'Enter') { handleLogin() } }} />
              <label className="dc-llbl">Senha</label>
              <input className="dc-linp" type="password" placeholder="..." value={loginPass} onChange={function (e) { setLoginPass(e.target.value) }} onKeyDown={function (e) { if (e.key === 'Enter') { handleLogin() } }} />
              ...
              <button className="dc-lbtn2" onClick={function () { setTela('cadastro'); setLoginErro('') }}>Criar conta</button>
            </div>
          )}
          {tela === 'cadastro' && (
            <div>
              ...
              <input className="dc-linp" type="text" placeholder="Seu Nome" value={cadNome} onChange={function (e) { setCadNome(e.target.value) }} />
              <label className="dc-llbl">Usuario</label>
              <input className="dc-linp" type="text" placeholder="seu.usuario" value={cadUser} onChange={function (e) { setCadUser(e.target.value) }} />
              <label className="dc-llbl">Senha</label>
              <input className="dc-linp" type="password" placeholder="Minimo 6 caracteres" value={cadPass} onChange={function (e) { setCadPass(e.target.value) }} />
              <label className="dc-llbl">Confirmar senha</label>
              <input className="dc-linp" type="password" placeholder="Repita a senha" value={cadPass2} onChange={function (e) { setCadPass2(e.target.value) }} onKeyDown={function (e) { if (e.key === 'Enter') { handleCadastro() } }} />
              ...
              <button className="dc-lbtn2" onClick={function () { setTela('login'); setCadErro(''); setCadOk('') }}>Voltar ao login</button>
            </div>
          )}
        </div>
      </div>
  ...
  if (!logado) {
    return (
      <TelaAuth
        ...
        onLogin={function (u: string, n: string) { setLogado(true); setUsuarioAtual(u); setNomeAtual(n) }}
      />
    )
  }

  const isDark = tema === 'dark'
  const passo = autenticado ? 1 : 2
  const initials = (nomeAtual || usuarioAtual).slice(0, 2).toUpperCase()
  let valorSel = 0
  selecionadas.forEach(function (i) { if (contas[i]) { valorSel += contas[i].valor } })
  let totalValor = 0
    ...
    <div className={'dc-app' + (isDark ? ' dc-dark' : '')}>
      <header className="dc-topbar">
        ...
        <div className="dc-topbar-right">
          {autenticado && <span className="dc-badge-conn">Conectado</span>}
          <div className="dc-icon-btn" onClick={toggleTema} title="Alternar tema">
      ...
      <div className="dc-content">
              ...
              <div className="dc-tw">
                <table className="dc-tbl">
                  <thead className="dc-thead">
                    <tr>
                      <th className="dc-th">Data</th>
                      <th className="dc-th">Empresa</th>
                      <th className="dc-th">Contas</th>
                      <th className="dc-th">Valor Total</th>
                      <th className="dc-th">OK</th>
                      <th className="dc-th">Erros</th>
                    </tr>
                  </thead>
                  <tbody>
        ...
        {abaAtiva === 'importar' && (
          <div>
            {!autenticado && (
              <div className="dc-card">
                <div className="dc-ct"><div className="dc-step">1</div>Conectar ao ContaAzul</div>
                <p style={{ color: 'var(--text2)', fontSize: '13px', marginBottom: '16px' }}>Clique no botao abaixo para autorizar o app a criar contas a pagar no ContaAzul.</p>
                <a href="/api/auth" style={{ display: 'inline-block', background: 'var(--accent)', color: 'white', padding: '10px 20px', borderRadius: '8px', fontWeight: 600, fontSize: '13px', textDecoration: 'none' }}>Entrar com ContaAzul</a>
              </div>
            )}
            <div className="dc-card">
              <div className="dc-ct"><div className="dc-step">{passo}</div>Importar planilha do Datacar</div>
              <div className="dc-up"
                onClick={function () { if (fileRef.current) { fileRef.current.click() } }}
                onDragOver={function (e) { e.preventDefault(); setDrag(true) }}
                onDragLeave={function () { setDrag(false) }}
                onDrop={function (e) { e.preventDefault(); setDrag(false); if (e.dataTransfer.files && e.dataTransfer.files[0]) { processarArquivo(e.dataTransfer.files[0]) } }}
              ...
              <input ref={fileRef} type="file" accept=".xlsx" style={{ display: 'none' }} onChange={function (e) { if (e.target.files && e.target.files[0]) { processarArquivo(e.target.files[0]) } }} />
              {uploadMsg !== null && (
            ...
            {contas.length > 0 && (
              <div>
                <div className="dc-card">
                  <div className="dc-ct"><div className="dc-step">{passo + 1}</div>Configurar lancamento</div>
                  <div className="dc-stats">
                    ...
                    <div className="dc-stat"><div className="dc-sv" style={{ fontSize: '13px', marginTop: '3px' }}>{empresa}</div><div className="dc-sl">Empresa</div></div>
                  </div>
                  <div className="dc-sels">
                    <div>
                      <div className="dc-fl">Categoria de despesa</div>
                      <select className="dc-sel" value={catId} onChange={function (e) { setCatId(e.target.value) }}>
                        <option value="">Sem categoria</option>
                        {categorias.map(function (c) { return <option key={c.id} value={c.id}>{c.name || c.description || c.id}</option> })}
                      </select>
                    </div>
                    <div>
                      <div className="dc-fl">Conta financeira</div>
                      <select className="dc-sel" value={contaFinId} onChange={function (e) { setContaFinId(e.target.value) }}>
                        <option value="">Sem conta especifica</option>
                        {contasFin.map(function (c) { return <option key={c.id} value={c.id}>{c.name || c.description || c.id}</option> })}
                      </select>
                    </div>
                  </div>
                </div>
                ...
                <div className="dc-card">
                  <div className="dc-ct"><div className="dc-step">{passo + 2}</div>Revisar e lancar</div>
                  <div className="dc-toolbar">
                    <label className="dc-chk-lbl">
                      <input type="checkbox" checked={selecionadas.size === contas.length && contas.length > 0} onChange={function (e) { toggleTodas(e.target.checked) }} style={{ width: '14px', height: '14px', accentColor: 'var(--accent)' }} />
                      Selecionar todas
                    </label>
                    ...
                    <table className="dc-tbl">
                      <thead className="dc-thead">
                        <tr>
                          ...
                          <th className="dc-th">Fornecedor</th>
                          <th className="dc-th">NF</th>
                          <th className="dc-th">Documento</th>
                          <th className="dc-th">Emissao</th>
                          <th className="dc-th">Vencimento</th>
                          <th className="dc-th">Valor</th>
                        </tr>
                      </thead>
                      <tbody>
                        {contas.map(function (c, i) {
                          return (
                            <tr key={i}>
                              <td className="dc-td"><input type="checkbox" checked={selecionadas.has(i)} onChange={function () { toggleSel(i) }} style={{ width: '14px', height: '14px', accentColor: 'var(--accent)' }} /></td>
            ...
            {resultados !== null && resumo !== null && (
              <div className="dc-card">
                ...
                <div className="dc-msg" style={{ marginTop: 0, marginBottom: '12px', background: resumo.erros === 0 ? 'var(--badge-ok)' : 'rgba(220,38,38,0.1)', borderLeftColor: resumo.erros === 0 ? '#16a34a' : '#dc2626', color: resumo.erros === 0 ? 'var(--badge-ok-text)' : 'var(--danger)' }}>
                  {resumo.ok} lancadas com sucesso
                  {resumo.erros > 0 && <span> | {resumo.erros} com erro</span>}
                </div>
